...
public:
  ...
  GLOGIT(List X_list0, List Y_list0, arma::uvec Z_prior, double psi_par,  double H, double alpha, double a_prior, double tau_prior, arma::vec b_prior, arma::mat Q_prior, int n_iter0);
...
//=========== CONSTRUCTOR =============//
GLOGIT::GLOGIT(List X_list0, List Y_list0, arma::uvec Z_prior,double psi_par, double H, double alpha, double a_prior, double tau_prior, arma::vec b_prior, arma::mat Q_prior, int n_iter0) : prior(alpha, H, X_list0.size())
{
  // Number of datastes
  n = X_list0.size();
  // Base partition
...
void GLOGIT::updatePolyaVars(int iter)
{
 for(int j = 0; j < n; ++j)
 {
...
//---------------------------------//
// update intercep term
//---------------------------------//
void GLOGIT::updateIntercept(int iter)
{
 for(int j = 0; j < n; ++j)
 {
...
//------------------------------------//
void GLOGIT::updateBeta(int iter)
{
...
//-----------------------------------------------------------/
void GLOGIT::updateClass(int iter)
{
  ...
  for(int j = 0; j < n; ++j)
  {
     ...
     for(int h = 0; h < H0; ++h)
     {
       ...
       log_lik = sum(Y_list(j)%reg  - log(1 + exp(reg)));
       prob_tmp(h) += log_lik;
     }
