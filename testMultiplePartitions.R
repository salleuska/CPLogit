...
#######################################
## Load simulated data
load("simulations/simulation_discrete.RData")
...
set.seed(89)
# res  = gibbsLogitCPMultiple(X_list, Y_list, as.matrix(Z_0), psi_par, H, H_upper, alpha, a_prior, tau_prior,  b_prior, Q_prior,  n_iter)
# burn = 1000
...
psi_par = 17
set.seed(9999)
Z_0 <- cbind(sample(0:4, 12, replace = T), c(0,1,1,2,2,2,3,3,3,0,0,0))

library(CPLogit)
res = gibbsLogitCPMultiple(X_list, Y_list, Z_0, psi_par, H , H_upper,
 alpha,  a_prior, tau_prior,  b_prior, Q_prior,  n_iter)
burn = 1000
# check classification
cl = getClassMAP(res$clustering, burn, tab = TRUE)
table(cl$class, Z_0)
table(cl$class, Z_0[,1])
table(cl$class, Z_0[,2])
